...
import { getLocalDateKey } from '../services/workoutService';
...
const ProgressView: React.FC<ProgressViewProps> = ({ log, onLogChange }) => {
    ...
    const chartData = useMemo(() => {
        ...
        const workoutLogs = log.filter(entry => !entry.title.toLowerCase().includes('descanso activo'));
    ...
    const logsForSelectedDate = useMemo(() => {
        ...
        return log.filter(entry => getLocalDateKey(new Date(entry.date)) === selectedDate);
    }, [log, selectedDate]);
