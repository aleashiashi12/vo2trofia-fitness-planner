...
export const logExerciseCompletion = async (dayId: DayId, title: string): Promise<void> => {
    try {
        ...
        // Find an entry for today, regardless of dayId, to consolidate logs.
        let entryForToday = log.find(entry => entry.date.startsWith(today));
...
export const addWorkoutLog = async (date: string, workout: {dayId: DayId, title: string}): Promise<ActivityLogEntry[]> => {
    try {
        ...
        const existingEntry = log.find(l => l.date.startsWith(dateKey));
        if (existingEntry) {
